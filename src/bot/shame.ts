...
const singleShameResponses = [
    (userName: string) => `🤢 <b>${userName}</b> вызывает рвотный рефлекс у спортивного сообщества.`,
    (userName: string) => `☠️ <b>${userName}</b> - позор в чистом виде.`,
    (userName: string) => `🧻 <b>${userName}</b> - туалетная бумага в мире железного спорта.`,
    (userName: string) => `🪵 Дерево за окном показало лучшие результаты, чем <b>${userName}</b>`,
    (userName: string) => `💩 <b>${userName}</b> - доказательство теории деградации видов.`,
    (userName: string) => `🫁 Легкие <b>${userName}</b> знают только один газ - пердёж матёрых зеков.`,
    (userName: string) => `🪙 Премия Дарвина для <b>${userName}</b> - за выживание без мышц.`,
    (userName: string) => `🍞 <b>${userName}</b> - заплесневелый сухарь в компании протеиновых батончиков.`,
    (userName: string) => `🦴 Мышечная масса <b>${userName}</b> сравнима с медузой в спирте.`,
    (userName: string) =>
...
const getGroupShameMessage = (losers: string[], count: number): string => {
    ...
    const phrases = [
        ...
        `🏆 <b>Командный зачет по лени</b>! ${count} человек доказали, что бездарность любит компанию: ${losers
            .slice(0, 3)
            .join(', ')}${count > 3 ? ' и еще ' + (count - 3) + ' лузера' : ''}`,
        `${groupEmoji} Когда ${losers[0]} чихнул бездельем, ${
            count - 1
        } подхватили! Вот она - сила коллектива!`,
        `💩 <b>Гора мусора выросла до ${count} единиц</b>: ${
            losers.length > 3 ? losers[0] + ', ' + losers[1] + ' и ко.' : losers.join(', ')
        }`,
        `🪓 <b>Лесоруб бы заплакал</b> от такой группы сухих пней: ${losers.slice(0, 2).join(', ')}${
            count > 2 ? ` и ${count - 2} других пеньков` : ''
        }`,
        `🧟 <b>Зомби-апокалипсис!</b> ${count} особей (${losers[0]}, ${losers[1]}${
            count > 2 ? ` и др.` : ``
        }) не могут оторвать жопы от дивана`,
    ];
